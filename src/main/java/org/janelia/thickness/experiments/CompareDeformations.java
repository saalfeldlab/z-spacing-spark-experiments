...
//        public final double
	...
	public static void comparePositions(
			...
			final int off,
			final int windowRange ) throws InterruptedException
	{
	...
	public static void compareGradients(
			final String groundTruthPattern,
			final String base,
			final ScaleOptions config ) throws InterruptedException
	{
...
//        }
		...
		for ( int z = start; z < stop; ++z )
		{
			...
			for ( Cursor< FloatType > compareC = compareImg.cursor(), gtC = gtImg.cursor(), t = target.cursor(), d = diff.cursor(); compareC.hasNext(); )
			{
...
//        System.out.println( Arrays.toString(m.getMatrix(matrix) ));
			...
			throws NotEnoughDataPointsException, IllDefinedDataPointsException
	{
		...
		for ( int z = start; z < stop; ++z )
		{
			...
			for ( Cursor< FloatType > c2 = img2.cursor(), c1 = img1.cursor(), d = diff.cursor(); c2.hasNext(); )
			{
